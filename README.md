...
- `*_audio_loudness.json`: `-CleanWav` 사용 시 원본/clean WAV loudnorm 측정값
...
현재 테스트에는 Windows URI, 한글/공백/특수문자 경로, UNC URI, output collision, 전체 pipeline limit 전달, transcript cache, rejected XML/review JSON, CLI 오류 메시지, FCP7 XML 파싱, mocked STT E2E가 포함됩니다.
