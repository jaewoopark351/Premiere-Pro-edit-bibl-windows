...
## 제작 모드 (소스에 따라 선택)
- **모드 A · 컷 기반(권장)** — 그 영상에 auto_cut 산출물(`_cut.xml`+`_cut_audio.wav`+`_words.json`)이 있으면:
  `python3 engine/shorts_cut.py <원본영상> "MM:SS-MM:SS" ...` (구간은 **컷 타임라인 기준** = `_cut.srt` 시각)
  → keep만 모은 **무음·숨소리 빠진 타이트한 9:16 XML** + 6~7자 SRT.
