...
**실행 모드:** 서브 에이전트 파이프라인 + 감독자(편집 디렉터 검수). 작업이 순차 의존적이고(전사→분석→기획→재컷→자막) 무거운 연산은 Windows 포팅본의 `bibl_windows` CLI가 결정적으로 처리하므로, 파일 기반 핸드오프가 가장 안정적이다.

모든 에이전트 호출은 `Agent` 도구 + `subagent_type` + **`model: "opus"`**. 산출물은 `output/_workspace/`에 파일로 주고받는다.

## Phase 0: 컨텍스트 확인
1. `output/_workspace/<base>/` 존재 여부 확인.
   - 없음 → **초기 실행** (전체 Phase)
...
## Phase 2: 러프컷 + 전사 (컷편집가)
- `Agent(cut-editor, model:opus)` → Windows CLI 실행(백그라운드, 완료 대기).
  ```powershell
  .\.venv\Scripts\python.exe -B -m bibl_windows.cli run "영상" --preset standard
  ```
- 산출: `_cut.xml/_cut_audio.wav/_cut.srt/.vtt/.ass/_report.html/_transcript.json/_transcript.md/_edit_diff.md/_manifest.json` + `output/_workspace/<base>/30_cut_result.md`.
- **여기서 전사(`_transcript.json` 또는 `_transcript.md`)가 먼저 나와야** 리서처·기획자가 일한다.
...
## Phase 5: 최종 컷 (컷편집가)
- `_content_cuts.json`이 생겼거나 기획의 프리셋/설정이 Phase 2와 다르면 `Agent(cut-editor, model:opus)`로 **재실행**한다. 현재 Windows CLI가 내용 컷 자동 반영을 지원하는지 먼저 확인하고, 미지원이면 `99_director_handoff.md`에 Premiere 수동 컷 지시로 남긴다.
- 둘 다 없으면 건너뜀.
...
## 데이터 흐름 (파일 기반)
```
00_claude_context → 00_director_brief → 10_research → 20_plan
        ↓              ↓            ↓
   [cut-editor CLI] → _transcript.json/_cut.srt/_report.html/30_cut_result
        ↓
   [subtitle-editor] → _cut.srt(교정)
        ↓
   99_director_handoff (← 전부 종합)
```
`output/_workspace/<base>/{단계}_{에이전트}_{산출물}` 컨벤션. 최종 산출물(_cut.xml/.srt/.wav)만 `output/`에, 중간물은 `_workspace/<base>/`에 보존(감사·재실행용). Windows CLI는 `00_claude_context.md`, `30_cut_result.md`, `99_director_handoff.md` 초안을 자동 생성한다.
...
## 테스트 시나리오
- **정상 흐름**: 73분 라이브 토크 → strict doctor → dry-run → brief(표준) → 러프컷(13% 제거) → 리서치(하이라이트 5·삭제추천 3) → 기획(인트로훅+표준유지) → 자막교정(닉네임 4건) → 핸드오프. 기대: `_cut.xml`+교정 SRT+핸드오프 노트.
